export const meta = {
  ...
  description:
    "A practitioner's guide to what a proper interior design and renovation contract should cover in Malaysia — scope, payment milestones, timelines, variation orders, and warranty.",
  keywords: [
...
A contract should say, in general terms, what happens if either side wants to end the agreement before completion, and how disagreements get handled if they can't be resolved directly between the parties. I'm deliberately keeping this section brief, because the specifics of what remedies are actually available to you depend on the exact terms of your contract and on Malaysian contract law, and that's a question for a lawyer, not a design blog. What I'd say generally is: a contract that says *nothing* about termination or disputes is worse than one that says something, even something general — it at least signals the other party has thought about what happens if things don't go smoothly.

{/* VERIFY: do not state specific legal remedies without a lawyer's review before publishing */}
