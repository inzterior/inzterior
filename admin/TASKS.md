...
- [ ] Client leads pipeline started — code shipped 2026-07-30 (see Notes for what's built and the setup steps still needed to activate it)
...
- Leads pipeline (2026-07-30): the Contact form and Estimate calculator's "email me this estimate" both now POST to `/api/leads`, which appends a row to a Google Sheet and sends an email notification via Resend. Previously they just built a `mailto:` link and relied on the visitor's own device having an email client configured — unreliable, no record kept, silently lost leads on many mobiles. Code is built and deployed but **inactive** until this one-time setup is done (~15 min, needs Fuyi):
  1. In Google Cloud Console (under `inzterior.my@gmail.com`): create a project, enable the Google Sheets API, create a Service Account, and generate/download its JSON key.
  2. Create a Google Sheet ("Inzterior — Leads") under `inzterior.my@gmail.com`, with a tab named `Leads` and header row `Timestamp | Source | Name | Email | Phone | Details`. Share it with the service account's email (the `...@...iam.gserviceaccount.com` address) as Editor — this is what gives it write access. Share it with Billy and Jackie's Google accounts too, for visibility.
  3. In Vercel (`invoice-now-check` team → `inzterior` project → Settings → Environment Variables), add: `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_PRIVATE_KEY` (from the JSON key), `GOOGLE_SHEET_ID` (from the Sheet's URL), `GOOGLE_SHEET_NAME` (`Leads`). See `.env.example` in the repo for the exact variable names and format.
  4. Optional but recommended — email notification on every new lead via Resend (resend.com, free tier): create an account, verify sending from the `inzterior.com` domain (or use their default `onboarding@resend.dev` sender to start), get an API key, and add `RESEND_API_KEY` (+ optionally `RESEND_FROM_EMAIL`, `LEADS_NOTIFY_EMAIL`) to Vercel. Without this, leads still save to the Sheet — you just won't get pinged immediately.
  5. Redeploy after adding the env vars for them to take effect.
  Until this is done, both forms will show the visitor a "we couldn't save your details — please email us directly" error rather than silently failing.
